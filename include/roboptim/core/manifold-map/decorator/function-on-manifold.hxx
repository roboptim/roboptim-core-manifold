...
namespace roboptim
{
  ...
  template <typename T>
  std::ostream&
  FunctionOnManifold<T>::print_ (std::ostream& o)
  {
  ...
  template <typename T>
  void
  FunctionOnManifold<T>::mapArgument (const_argument_ref argument)
    const
  {
    for (long i = 0; i < this->mappingFromFunctionSize_; ++i)
      {
  ...
  template <typename T>
  void
  FunctionOnManifold<T>::impl_compute
  ...
  template <typename T>
  void
  FunctionOnManifold<T>::unmapTangentJacobian(mnf::RefMat jacobian)
    const
  {
  ...
  template <typename T>
  void
  FunctionOnManifold<T>::unmapGradient(gradient_ref gradient) const
  {
  ...
  template <typename T>
  void
  FunctionOnManifold<T>::impl_gradient (gradient_ref gradient,
  ...
  template <typename T>
  void
  FunctionOnManifold<T>::impl_jacobian (jacobian_ref jacobian,
					const_argument_ref argument)
    const
  {
    this->mapArgument(argument);
  ...
  template <typename T>
  void
  FunctionOnManifold<T>::manifold_jacobian (mnf::RefMat jacobian,
					    const_argument_ref argument)
    const
  {
    this->mapArgument(argument);
  ...
  template<typename T>
  void
  FunctionOnManifold<T>::unmapHessian(hessian_ref hessian) const
  {
  ...
  template<typename T>
  void
  FunctionOnManifold<T>::impl_hessian(hessian_ref hessian, const_argument_ref argument, size_type functionId) const
  {
    this->mapArgument(argument);
    hessian.setZero();
  ...
  template<typename T>
  void
  FunctionOnManifold<T>::unmapJacobian(jacobian_ref jacobian) const
  {
  ...
  template <typename T>
  const mnf::Manifold*
  FunctionOnManifold<T>::getManifold() const
  {
    return this->manifold_;
  }

  template <typename T>
  typename FunctionOnManifold<T>::flag_t
  FunctionOnManifold<T>::getFlags() const
  {
